...
def contact(request):
	if request.session.get('user'):
		...
		return render(request,'contact.html',{'email':request.session.get('user'),'error':error})
def expense(request):
	if request.session.get('user'):
		email=request.session.get('user')
...
def dashboard(request):
	if request.session.get('user'):
		...
		if request.session.get('error'):
			error=request.session.get('error')
			del request.session['error']
		res1=expense_detail.objects.filter(email=request.session.get('user'))
		todays_date = date.today()
...
def myprofile(request):
	if request.session.get('user'):
		email=request.session.get('user')
		res=user_detail.objects.filter(email=email)
		errors=""
		if request.session.get('error'):
...
def deleteaccount(request):
	if request.session.get('user'):
		email=request.session.get('user')
		user_detail.objects.filter(email=email).delete()
		expense_detail.objects.filter(email=email).delete()
		query.objects.filter(email=email).delete()
		replies.objects.filter(email=email).delete()
...
def userquery(request):
	if request.session.get('user'):
		res=query.objects.filter(email=request.session.get('user'))
